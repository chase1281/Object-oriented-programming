...
void MusicManager::save(string newfilename) {
    ofstream outfile;
    outfile.open(newfilename);
    for (auto plist : playlists) {
        outfile << "PlayList: " << plist->get_name() << endl;
        for (const auto& song : plist->get_songs()) {
            outfile << song->get_title() << "," << song->get_artist() << ","
                << song->get_album() << "," << song->get_url() << endl;
        }
        outfile << endl;
    }
    outfile.close();
